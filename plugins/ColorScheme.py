...
# Released on 28/05/21 (DD/MM/YY).
# Made by Rikko.

# Feel free to copy/do whatever with the code you like. You don't even
...
class ColorSchemeWindow(ba.Window):
    ...
    def draw_ui(self):
        ...
        self._height = height = (
            275.0 if uiscale is ba.UIScale.SMALL else
            288.0 if uiscale is ba.UIScale.MEDIUM else 300.0)
        spacing = 40
        ...
        reset_button = ba.buttonwidget(parent=self._root_widget,
                                            position=(width - (177 + x_inset),
                                                      height - 60),
                                            size=(155, 60),
                                            color=(0.2, 0.5, 0.6),
        ...
        ba.buttonwidget(edit=self._color_button,
                        on_activate_call=ba.Call(self._pick_color, "color"))
    ...
    def _pick_color(self, tag):
        ...
        ColorPicker(parent=None, position=(0,0), initial_color=initial_color, delegate=self, tag=tag)
...
class CustomTransactions:
    ...
    def _handle(self, transaction, *args, **kwargs):
        for transaction_code, transaction_fn in self.custom_transactions.items():
            if transaction.get("code") == transaction_code:
...
class Main(ba.Plugin):
    def __init__(self):
        if _ba.env().get("build_number", 0) >= 20258:
            ...
            print("ColorScheme.py only runs with BombSquad versions higher than 1.5.29.")
