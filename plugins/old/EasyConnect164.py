...
# REQUIREMENTS
# built for bs 1.6.4 and above 

# by Mr.Smoothy for Bombsquad version 1.6
...
def new_build_favorites_tab(self, region_height: float) -> None:
        ...
        ba.textwidget(parent=self._container,
                        position=(90 if uiscale is ba.UIScale.SMALL else 120,btnv+ 120 if uiscale is ba.UIScale.SMALL else btnv+90),
                        size=(0,0),
                        h_align='center',
                        color=(0.8,0.8,0.8),
                        v_align='top',
        ...
        self.retry_inter_text=ba.textwidget(parent=self._container,
                        position=(90 if uiscale is ba.UIScale.SMALL else 120,btnv+25),
                        size=(0,0),
                        h_align='center',
                        color=(0.8,0.8,0.8),
                        v_align='center',
...
def new_on_favorites_connect_press(self) -> None:
        if self._favorite_selected is None:
            ...
            if self.retry_inter >0 and (_ba.get_connection_to_host_info()=={} or _ba.get_connection_to_host_info()['build_number']==0):
                ...
                self._retry_timer = ba.Timer(self.retry_inter,ba.Call(self._on_favorites_connect_press),timetype = ba.TimeType.REAL)
...
def update(self, index: int, party: PartyEntry, sub_scroll_width: float,
        ...
        if party.stats_addr or True:
            ...
            self._stats_button = ba.buttonwidget(
                ...
                scale=0.9)
            ba.buttonwidget(edit=self._stats_button,on_activate_call=ba.Call(self.on_stats_click,self._stats_button,party))
...
def on_stats_click(self,widget,party):
	...
	DisChoices=[ba.Lstr(resource="ipp",fallback_value = "Connect by IP"),ba.Lstr(resource="copy id",fallback_value = "Copy Queue ID"),ba.Lstr(value="Save")]
	if party.stats_addr:
...
class PartyQuickConnect(ba.Window):
    def __init__(self, address: str, port: int):
        ...
        self.move_R=ba.Timer(0.01,ba.Call(self.move_right),timetype=ba.TimeType.REAL,repeat=True)
    def move_right(self):
    ...
    def connect(self,address,port):
        ...
        if not self.closed and (_ba.get_connection_to_host_info()=={} or _ba.get_connection_to_host_info()['build_number']==0):
                ...
                self._retry_timer = ba.Timer(1.5,ba.Call(self.connect,address,port),timetype = ba.TimeType.REAL)
    def close(self) -> None:
        """Close the ui."""
        self.closed=True
...
class InitalRun(ba.Plugin):
    def __init__(self):
        if _ba.env().get("build_number",0) >= 20386:
            replace()
        else:print("EasyConnect only runs with BombSquad version higer than 1.6.4")