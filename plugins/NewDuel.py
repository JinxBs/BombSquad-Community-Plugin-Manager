...
class NewDuelGame(ba.TeamGameActivity[Player, Team]):
    ...
    def _clear_all_objects(self) -> None:
        ...
        for player in self.players:
            if player.playervs1 or player.playervs2:
                ...
                ba.timer(0.5, new_region.delete)
                ba.emitfx(position=player.node.position,
                          velocity=(0,0,0),
                          count=16,
                          scale=0.8,
                          spread=1.5,
                    ...
                    if not player.is_alive():
                        ...
                        player.actor.handlemessage(
                            ba.StandMessage(
                                pos1[0] if player.playervs1 else pos2[0],
                                pos1[1] if player.playervs1 else pos2[1]))
                    if self._first_countdown:
                        delay = 4.0
                    else:
                        delay = 0.5
                    ba.timer(delay, self._countdown)
                    ba.timer(0.1, self._clear_all_objects)
    ...
    def _countdown(self) -> None:
        ...
        if self._fight_delay == 0:
            ...
            ba.timer(self._fight_delay, self.count3)
    ...
    def handlemessage(self, msg: Any) -> Any:
        ...
        if isinstance(msg, ba.PlayerDiedMessage):
            ...
            if player.playervs1:
                ...
                player.in_game = False
                self.spawn_order.append(player)
            ba.timer(0.2, self._update_order)
